...
export const LanguageString: React.FC<
  { [key: string]: any } & { as?: string | React.FC<any>; language: string }
> = ({ as: Component, language, children, ...props }) => {
  const { i18n } = useTranslation();

  const viewingDirection = useMemo(
    () => (i18n.dir ? i18n.dir(language) : "ltr"),
    [language]
  );
...
function getClosestLanguage(
  ...
  // Root match (en-us === en)
  const root =
    i18nLanguage.indexOf("-") !== -1
      ? i18nLanguage.slice(0, i18nLanguage.indexOf("-"))
      : null;
...
export const useClosestLanguage = (
  getLanguages: () => string[],
  deps: any[] = []
): string | undefined => {
  const { i18n } = useTranslation();
...
export function useLocaleString(
  ...
  const language = useClosestLanguage(
    () => Object.keys(inputText || {}),
    [inputText]
  );
  return [
    useMemo(() => {
