  ...
  return (
    <ExpandTabContainer>
      {editorContext?.view === "noNav" ? (
        <SmallButton aria-label="expand" onClick={() => editorContext?.setView("tree")} title="Navigation panel">
          <DownIcon rotate={270} />
        </SmallButton>
      ) : (
        <SmallButton aria-label="close" onClick={() => editorContext?.setView("noNav")} title="Close navigation panel">
          <DownIcon rotate={90} />
        </SmallButton>
      )}
    </ExpandTabContainer>
