...
export const metadataEditorReducer = produce(
  (state: MetadataEditorState, action: MetadataEditorActions) => {
    switch (action.type) {
      case "SELECT_ITEM": {
        // Toggle.
        state.selected =
          state.selected === action.payload.id ? undefined : action.payload.id;
        break;
      }
      ...
      case "REMOVE_ITEM": {
        ...
        if (state.modified.indexOf(action.payload.id) !== -1) {
          state.modified = state.modified.filter(
...
export const valuesToIntlString = (
...
export const intlStringToValues = (
  intlStr: InternationalString,
  key: string
): MetadataDefinition[] => {
  if (!intlStr) {
    return [];
...
export function useMetadataEditor({
  ...
  const closestLang = useClosestLanguage(
    () => fieldKeys.map((key) => state.fields[key].language),
    [state.fields]
  );
