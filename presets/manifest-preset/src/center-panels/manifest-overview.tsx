...
export function ManifestOverviewCenterPanel() {
  ...
  const [size, setSize] = useLocalStorage<"grid-sm" | "grid-md" | "grid-lg">("manifest-grid-size", "grid-md");
    ...
    <>
      <div className="p-2 flex gap-2 justify-between items-center">
        <div className="flex gap-2 items-center">
          ...
          <ActionButton primary={size === "grid-sm"} onPress={() => setSize("grid-sm")}>Small</ActionButton>
          <ActionButton primary={size === "grid-md"} onPress={() => setSize("grid-md")}>Medium</ActionButton>
          <ActionButton primary={size === "grid-lg"} onPress={() => setSize("grid-lg")}>Large</ActionButton>
        </div>
