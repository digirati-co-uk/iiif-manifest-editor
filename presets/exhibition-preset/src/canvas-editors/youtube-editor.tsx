...
import { type CanvasEditorDefinition, useGenericEditor, useLayoutActions } from "@manifest-editor/shell";
...
export const youtubeMainEdtior: CanvasEditorDefinition = {
  ...
  component: (strategy) =>
    strategy.type === "media" ? (
      <YouTubeEditor strategy={strategy as MediaStrategy} video={strategy.media as SingleYouTubeVideo} />
    ) : null,
  supports: {
...
export function YouTubeEditor({ strategy, video }: { strategy: MediaStrategy; video: SingleYouTubeVideo }) {
  ...
  const ytThumbnail = `https://img.youtube.com/vi/${media.youTubeId}/maxresdefault.jpg`;

  return (
    <div className="overflow-y-auto">
      <div className="p-8">
        <div className="bg-black z-20 flex p-8 rounded-lg max-w-full max-h-[70vh] mx-auto" style={{ aspectRatio: ar }}>
          <iframe
            title="YouTube video"
      ...
      <div className="bg-white shadow w-full p-8 flex gap-8">
        <div>
          ...
          >
            {canvas.thumbnail?.length ? (
              ...
              <div className="text-white/50 text-center py-16">No thumbnail</div>
            )}
          </div>
        </div>
        <div className="flex-1">
          <LocaleString
            ...
            defaultText={(<span className="text-black/30">No Label</span>) as any}
          >
          ...
          <LocaleString
            as="p"
            className="mt-2 mb-8"
            defaultText={(<span className="text-black/30">No summary</span>) as any}
          >
            {annotation.descriptive.summary.get()}
          </LocaleString>
          <ActionButton onPress={() => edit(annotation.ref())}>Edit properties</ActionButton>
        </div>
      </div>
    </div>
  );
