...
describe("getRegionIntersection", () => {
  test("returns the overlapping canvas region", () => {
    expect(
      getRegionIntersection({ x: 10, y: 10, width: 50, height: 40 }, { x: 40, y: 20, width: 50, height: 20 }),
    ).toEqual({ x: 40, y: 20, width: 20, height: 20 });
  });

  test("returns null when regions do not overlap", () => {
    expect(
      getRegionIntersection({ x: 0, y: 0, width: 10, height: 10 }, { x: 10, y: 0, width: 10, height: 10 }),
    ).toBeNull();
  });
});
...
describe("imageUrlWithRegion", () => {
  test("applies an Image API selector region to a thumbnail URL", () => {
    ...
    expect(imageUrlWithRegion("https://example.org/iiif/image/full/256,/0/default.jpg", getImageApiRegion(body))).toBe(
      "https://example.org/iiif/image/10,20,300,400/256,/0/default.jpg",
    );
  });

  test("applies crop and rotation to the generated Image API request", () => {
    const body = {
      selector: {
        type: "ImageApiSelector",
        region: "10,20,300,400",
    ...
    expect(getImageApiRotation(body)).toBe("90");
    expect(
      imageUrlWithTransform(
        "https://example.org/iiif/image/full/256,/0/default.jpg",
        getImageApiRegion(body),
...
describe("shouldUseComplexCanvasThumbnail", () => {
  ...
  test("retains existing complex cases and rejects malformed selectors", () => {
    ...
    expect(
      shouldUseComplexCanvasThumbnail({ type: "images", images: [image({ type: "Image" })] }, undefined, {
        x: 0,
        y: 0,
        width: 100,
        height: 100,
