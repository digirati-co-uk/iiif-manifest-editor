  ...
  return (
    <div className="relative w-full mx-auto max-w-screen-2xl lg:flex lg:items-start">
      <div
        ...
        className="sticky hidden border-r border-gray-200 top-16 shrink-0 lg:block"
      >
        <div className="h-full p-8 pl-16 -ml-3 overflow-y-auto">
          <DocsNavigation tree={thing.tree} activePath={activePath} />
        </div>
        <div className="absolute inset-x-0 top-0 h-8 bg-gradient-to-t from-white/0 to-white/100" />
        <div className="absolute inset-x-0 bottom-0 h-8 bg-gradient-to-b from-white/0 to-white/100" />
      </div>

      <div className="relative w-full grow">
        <DocsHeader tree={thing.tree} breadcrumbs={thing.breadcrumbs} title={doc.title} />
        <div className="w-full max-w-3xl p-4 pb-8 mx-auto mb-4 prose docs prose-slate prose-violet shrink prose-headings:font-semibold prose-a:font-normal prose-code:font-normal prose-code:before:content-none prose-code:after:content-none prose-hr:border-gray-200 md:mb-8 md:px-8 lg:mx-0 lg:max-w-full lg:px-16">
              ...
              <div className="grid grid-cols-1 gap-6 mt-12 md:grid-cols-2">
                {thing.childrenTree.map((card: any, index: number) => (
                  <Link key={index} href={card.urlPath} className="cursor-pointer no-underline">
                    <Card className="h-full p-6 py-4 hover:border-violet-100 hover:bg-violet-50">
