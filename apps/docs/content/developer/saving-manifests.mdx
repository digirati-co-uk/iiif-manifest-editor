...
const saveVault = useCallback(() => {
  // Returns a Presentation 4 Manifest.
  const data = vault.toPresentation4({ id: manifestId, type: 'Manifest' });
...
Presentation 3 should be treated as a compatibility export for viewers that do not yet support
Presentation 4. Some Presentation 4 features, including Scenes and 3D annotations, cannot be
downgraded without losing information.
