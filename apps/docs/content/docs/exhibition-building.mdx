...
## Theming the Exhibition Viewer

When you select the Manifest itself rather than an individual slide, the right hand metadata panel now includes a `Theme` tab. This tab stores a custom IIIF `service` on the Manifest containing optional exhibition theme and viewer defaults.

The theme editor allows you to:

- Choose a base preset
- Start from the built-in `Delft`, `Gallery`, or `Minimal` presets
- Adjust shared typography
- Change Delft and Scroll viewer color tokens
- Set default viewer options for exhibition, presentation, slideshow and scroll modes

These settings are optional. If no theme service is stored, the Exhibition Viewer falls back to its built-in defaults.
