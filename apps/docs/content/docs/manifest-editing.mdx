...
<video
  src="/Navigating to Canvas view.mp4"
  autoPlay
  loop
  muted
  controls
...
![](../../public/manifest-editing-overview-deep-zoom-1.png)
...
The right metadata panel with tabs for each aspect of the IIIF specification, presents the Manifest or Canvas metadata depending on what you have selected to edit, and this allows you to create, edit or update labels and values for the relevant properties.

### Example - Adding a required statement and rights

This example demonstrates adding a required statement to a Manifest.

<video
  src="/required-statement.mp4"
  autoPlay
  loop
  muted
  controls
...
As you create your new Manifest or edit an existing Manifest, you may want to change how the canvases in that manifest are ordered, re-ordering or removing canvases.

![Adjusting the order of canvases](../../public/manifest-editing-overview-edit-canvas-order-1.png)

To edit and change the structure of your Manifest, select the 'Edit Canvases' link to access the controls in both the list or grid views. Use the '=' icon to select and drag a canvas to a specific location in the list or use the options available via the 'three dots' icon.

![Use the three dots menu](../../public/manifest-editing-overview-edit-canvas-order-2.png)

## Previewing a change

When you make changes to your Manifest, you can preview them in compatible configured viewers. The available list is filtered by Presentation API support: Universal Viewer, Mirador and other Presentation 3 viewers are offered for compatible Manifests, while Presentation 4 features such as Scenes use Theseus or the raw Manifest preview.
