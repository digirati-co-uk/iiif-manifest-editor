...
Alternativly, if you click on the canvas list icon on the left side, you can add a new canvas from the sidebar.

![](/add-canvas.png)

## Import options

When you add a new content you are adding a new IIIF Canvas to the Manifest. A Cavnas can have multiple types. The most common type is an image, but you can also add a video or audio file.
