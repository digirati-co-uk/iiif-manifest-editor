...
import { usePolygonHelper } from "./use-polygon-helper";
...
export function useSvgEditor(options: SvgEditorOptions) {
  ...
  const { helper, state, currentShape } = usePolygonHelper(
    (renderState: RenderState, slowState: SlowState) => {
      svgHelpers.updateTransitionBoundingBox(
        transitionBoundingBox.current,
        renderState,
        slowState,
      );
      svgHelpers.updateBoundingBoxPolygon(
        boundingBox.current,
        renderState,
        slowState,
      );
      svgHelpers.updateTransitionShape(
        transitionShape.current,
        renderState,
        slowState,
      );
      svgHelpers.updateClosestLinePointTransform(
        hint.current,
        renderState,
        slowState,
      );
      ...
      svgHelpers.updateDrawPreview(
        transitionDraw.current,
        renderState,
        slowState,
        3,
      );
  ...
  const isHoveringPoint =
    ...
    state.actionIntentType === "select-point";
  ...
  const isStamping =
    state.transitioning &&
    state.selectedStamp &&
    state.transitionIntentType === "stamp-shape";
  ...
  const editor =
    currentShape && Shape ? (
      <>
        ...
        {state.transitioning &&
        state.transitionIntentType === "select-multiple-points" ? (
          <rect
            ref={selectBox}
