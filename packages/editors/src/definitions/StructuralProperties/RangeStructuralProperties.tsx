...
import {
  ...
  InputLabel,
  InputLabelEdit,
} from "@manifest-editor/ui/editors/Input";
...
export function RangeStructuralProperties() {
  ...
  const [canCreateRange, rangeActions] = useCreator(
    resource?.resource,
    "items",
    "Range",
  );
        ...
        <>
          <InputContainer $wide>
            ...
            <RangeList
              ...
              onSelect={(item, idx) =>
                toRef(item)?.type === "Canvas"
                  ? canvasActions.edit(item)
                  : rangeActions.edit(item)
              }
          ...
          {canCreateRange ? (
            <Button onClick={() => rangeActions.create()}>Add range</Button>
          ) : null}
        </>
      ) : null}
    </PaddedSidebarContainer>
